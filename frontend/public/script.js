...
class MemeBuilder {
  ...
  _drawAll() {
    ...
    for (const l of this.layers) {
      ...
      // render text with independent X/Y scale
      this.ctx.save();
      this.ctx.translate(l.x, l.y);
      this.ctx.scale(l.scaleX, l.scaleY);
      this.ctx.font   = `${l.baseHeight}px "${l.family}",sans-serif`;
      ...
      if (l.outlineWidth) {
        ...
        this.ctx.strokeText(l.text, 0, 0);
      }
  ...
  _doResize(pos) {
    ...
    if (L.type === 'text') {
      ...
      L.scaleX = L.width  / L.baseWidth;
      L.scaleY = L.height / L.baseHeight;
    }
    this._drawAll();
  }
...
const g = new GIF({
      workers: 2,
      quality: 10,
      // must be absolute so the browser can fetch it from /public/lib/…
